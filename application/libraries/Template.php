 ...
 *
 * @package CodeIgniter
 * @author  ExpressionEngine Dev Team, a little modification by https://roytuts.com
...
class Template {
    ...
    function __construct() {
        // Copy an instance of CI so you can use the entire framework.
    ...
    function set_regions($regions) {
        if (count($regions)) {
            ...
            foreach ($regions as $key => $region) {
                // Regions must be arrays, but you take the burden off the template
    ...
    function write($region, $content, $overwrite = FALSE) {
        if (isset($this->regions[$region])) {
            if ($overwrite === TRUE) { // Should you append the content or overwrite it
    ...
    function write_view($region, $view, $data = NULL, $overwrite = FALSE) {
        ...
        // Do you have more view suggestions?
        if (count($args) > 1) {
    ...
    function render($region = NULL, $buffer = FALSE, $parse = FALSE) {
        ...
        // Build the output array
            ...
            if ($this->parse_template === TRUE or $parse === TRUE) {
                ...
                // Parsers never handle output, but you need to mimick it in this case
                if ($buffer === FALSE) {
        ...
        if (isset($region['wrapper'])) {
            ...
            if (isset($region['attributes']) && is_array($region['attributes'])) {
                foreach ($region['attributes'] as $name => $value) {
                    // You don't validate HTML attributes. Imagine someone using a custom XML template..
                    $output .= "$name=\"$value\"";
                }
            }
